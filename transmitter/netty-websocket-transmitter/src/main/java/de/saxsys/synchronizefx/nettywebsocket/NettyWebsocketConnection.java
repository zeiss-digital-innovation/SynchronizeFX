...
class NettyWebsocketConnection extends SimpleChannelInboundHandler<Object> {
     ...
     * @param headerParams
     *            additional HTTP header parameters that should be send to the server on connection. This can be
     *            {@code null} if no special headers should be send.
     */
    public NettyWebsocketConnection(final URI uri, final NettyWebsocketClient parent,
            final Map<String, Object> headerParams) {
        this.wsHandshaker = WebSocketClientHandshakerFactory.newHandshaker(uri, WebSocketVersion.V13, PROTOCOL, false,
                createHttpHeaders(headerParams), Integer.MAX_VALUE);
        this.parent = parent;
    }
