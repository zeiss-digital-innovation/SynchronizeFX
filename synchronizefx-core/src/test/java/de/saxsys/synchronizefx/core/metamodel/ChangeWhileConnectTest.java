 ...
 * 
 * As the user should not be constrained to do only {@code synchronize}d changes
 * on his domain model, it is hard to ensure that the model is kept synchronous
 * while a client is connected. That is because at the time of writing this
 * test, the messages to reproduce the current state of the domain model are
 * produced by walking through the whole domain model via reflection. This
 * messages are than send to a connecting client. When changes on the server
 * side are done while this "walking" process is active to the part which has
 * already been walked through, this changes are lost. The purpose of this test
 * is to reproduce this problem and show if it has been solved or not.
 * 
 */
public class ChangeWhileConnectTest {
	...
	/**
	 * Changes the domain model while it is walked through and checks that
	 ...
	 * 
	 * <p>
	 * Assume that walker is already at C. When changes are done to B the walker
	 * won't get them as it already has visited B. This test checks if this
	 * changes are lost (which shouldn't happen).
	 * </p>
	 */
	@Test
	...
	/**
	 * Makes a change the domain model (remove) while it is walked through in
	 * the part that wasn't visited yet. It then checks that this change is
	 * merged correctly so that it won't result in errors.
	 * 
	 * <p>
	 * Assume the following object graph.
	 * </p>
	 ...
	 * 
	 * <p>
	 * This test produces a change that removes C while the walker is at A. This
	 * change is safed until the walking has finished. The walker did'nt create
	 * a message to create C. If the message to remove C would be send to the
	 * client, an error would occur that an unknown object should be removed.
	 * This test checks if this happens.
	 * </p>
	 */
	@Test
	...
	/**
	 * This list ensures that {@link ConcurrentModificationException} thrown by
	 * a list iterator in the property walker don't result in incorrect results.
	 */
	@Test
	...
	/**
	 * Tests if changes that occurred after the property walking has finished
	 * but before the commands are send are lost.
	 */
	@Test
	...
	/**
	 * Starts the domain model walking in a separate {@link Thread} and blocks
	 * the current {@link Thread} until {@link BlockingIntegerProperty} wakes
	 * it.
	 * 
	 * The commands created by this {@link Thread} are safed to
	 * {@link ChangeWhileConnectTest#commands}.
	 * 
	...
	private Thread startPropertyVisitorThread() {
		Thread propertyVisitorThread = new Thread() {
			...
			public void run() {
				...
				commands = EasyCommandsForDomainModel
						.commandsForDomainModel(meta);
			}
		};
		synchronized (threadWaitMonitor) {
			Thread.currentThread().setName("Test Thread");
	 ...
	 * {@link ChangeWhileConnectTest#startPropertyVisitorThread()} and wait for
	 * it to finish.
	 * 
	...
	/**
	 * Executes code in a new thread and waits until this new thread goes to
	 * sleep.
	 * 
	 * <p>
	 * The test cases in this class block the Property Walker thread until they
	 * allow it to continue. The also change property which causes the
	 * SynchronizeFX listeners to get invoked. These listeners will block the
	 * test thread until the property walking finishes. This results in a dead
	 * lock.
	 * </p>
	 * <p>
	 * Therefore this method does the property changing in a new thread and
	 * waits for until it goes to sleep. This ensures that the property value
	 * has changed bevor this method returns.
	 * </p>
	 * 
	 * @param runnable
	 *            The code to execute.
	...
	/**
	 * Executes the generated commands and checks if the resulting domain model
	...
	private static class Domain {
		final ListProperty<Domain> list = new SimpleListProperty<>(
				FXCollections.<Domain> observableArrayList());
		...
		public String toString() {
			return "Domain [list=" + list + ", waitingProperty="
					+ waitingProperty + "]";
		}
		...
		public int hashCode() {
			...
			result = prime
					* result
					+ ((waitingProperty == null) ? 0 : waitingProperty
							.hashCode());
			return result;
		}
	...
	/**
	 * Blocks a the executing thread after
	...
	private class BlockingIntegerProperty extends SimpleIntegerProperty {
		 ...
		 * {@link ChangeWhileConnectTest#waitAfterInvocationCount} reaches 0.
		 * 
		 * Each invocation decreases
		 * {@link ChangeWhileConnectTest#waitAfterInvocationCount} by 1.
		 */
	...
	/**
	 * Blocks the thread this class is executed in, when
	 * {@link CommandsForDomainModelCallback#sendCommands(List)} is called.
	 * 
	 */
	private class BlockingCommandsForDomainModelCallback implements
